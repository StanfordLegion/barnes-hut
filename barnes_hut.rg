...
task run_iteration(all_bodies : region(body), boundaries : region(boundary), quads : region(ispace(int1d), quad), conf : Config, sector_precision : uint, quad_ranges : region(ispace(int1d), rect1d), quad_sizes : region(uint), num_quads : uint)
  where
  reads writes(all_bodies),
  reads writes(boundaries),
  reads writes(quads),
  reads writes(quad_ranges),
  reads writes(quad_sizes)
do
  ...
  var quad_range_by_sector = partition(equal, quad_ranges, ispace(int1d, sector_precision * sector_precision + 1)) 
...
task main()
  ...
  var quad_ranges = region(ispace(int1d, sector_precision * sector_precision + 1), rect1d)
  ...
  for t=0,conf.time_steps do
      ...
      run_iteration(all_bodies, boundaries, quads, conf, sector_precision, quad_ranges, quad_sizes, num_quads)
      var iter_end = c.legion_get_current_time_in_micros()
      c.printf("Iteration time: %d ms\n", (iter_end - iter_start) / 1000)

      if conf.csv_dir_set then
        print_bodies_csv_update(all_bodies, conf, t+1)
      end

      if conf.svg_dir_set then
        print_bodies_svg(all_bodies, boundaries, conf, t+1)
      end
  end
